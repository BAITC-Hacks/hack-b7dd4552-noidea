...
export function DiscoveryPanel({turbine,onLoaded}:{turbine?:Turbine;onLoaded:()=>Promise<void>}){
  const [site,setSite]=useState(''),[purpose,setPurpose]=useState('history'),[plan,setPlan]=useState<Plan|null>(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
  const stop=useRef(false);
  const storageKey='wind-discovery-'+(turbine?.id??'none');
  ...
  return <div className="discovery"><h3>Подобрать данные по сайту</h3>
    ...
    <form className="filters" onSubmit={e=>{e.preventDefault();void run();}}>
      <label>Сайт или страница документации<input required placeholder="open-meteo.com" value={site} disabled={busy} onChange={e=>setSite(e.target.value)}/></label>
      <label>Для чего нужны данные<select disabled={busy} value={purpose} onChange={e=>setPurpose(e.target.value)}><option value="history">Проверить историю и время</option><option value="historical_forecast">Исторические прогнозы для ML</option><option value="forecast">Текущий прогноз погоды</option></select></label>
      <button disabled={busy||!turbine?.has_data}>{busy?'Выполняется…':'Найти и проверить запрос'}</button>
    </form>
    ...
    {plan&&<div><p><strong>{plan.status==='ready'?'Запрос проверен':plan.status==='complete'?'Загрузка завершена':'Нужно уточнение'}</strong></p><p>{plan.explanation}</p>
      ...
      {plan.status==='ready'&&<div className="filters"><button type="button" disabled={busy} onClick={()=>void load()}>{plan.completed.length?'Продолжить загрузку':'Загрузить весь период'}</button>{busy&&<button type="button" onClick={()=>{stop.current=true;}}>Остановить после текущей части</button>}</div>}
